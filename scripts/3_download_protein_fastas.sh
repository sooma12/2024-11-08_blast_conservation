...
awk 'NR > 1 && /^GCF/ {print $1, $2}' "$METADATA_EXTRACTED_TXT" | while read -r accession organism_name; do
    ...
    datasets download genome accession "$accession" --include protein --filename "${output_file}.zip"
